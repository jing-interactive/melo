/* miniz.c 2.1.0 - public domain deflate/inflate, zlib-subset, ZIP reading/writing/appending, PNG writing
   ...
   * Low-level Deflate/Inflate implementation notes:
     ...
     Compression: Use the "_tdefl" API's. The compressor supports raw, static, and dynamic blocks, lazy or
     greedy parsing, match length filtering, RLE-only, and Huffman-only streams. It performs and compresses
     approximately as well as zlib.

     Decompression: Use the "_tinfl" API's. The entire decompressor is implemented as a single function
     coroutine: see _tinfl_decompress(). It supports decompression into a 32KB (or larger power of 2) wrapping buffer, or into a memory
     block large enough to hold the entire file.

     The low-level _tdefl/_tinfl API's do not make any use of dynamic memory allocation.
   ...
   * PNG writing: See the _tdefl_write_image_to_png_file_in_memory() function, originally written by
   ...
   * ZIP archive API notes:
     ...
     - Archive reading: Just call this function to read a single file from a disk archive:
      ...
      void *miniz_zip_extract_archive_file_to_heap(const char *pZip_filename, const char *pArchive_name,
        size_t *pSize, miniz_uint zip_flags);

     For more complex cases, use the "miniz_zip_reader" functions. Upon opening an archive, the entire central
     ...
     Alternately, you can iterate through all the files in an archive (using miniz_zip_reader_get_num_files()) and
     retrieve detailed info on each file by calling miniz_zip_reader_file_stat().

     - Archive creation: Use the "miniz_zip_writer" functions. The ZIP writer immediately writes compressed file data
     ...
     - Archive appending: The simple way to add a single file to an archive is to call this function:
      ...
      miniz_bool miniz_zip_add_mem_to_archive_file_in_place(const char *pZip_filename, const char *pArchive_name,
        const void *pBuf, size_t buf_size, const void *pComment, miniz_uint16 comment_size, miniz_uint level_and_flags);
     ...
     1. The safest way is to use a miniz_zip_reader to read the existing archive, cloning only those bits you want to
     preserve into a new archive using using the miniz_zip_writer_add_from_zip_reader() function (which compiles the
     compressed file data as-is). When you're done, delete the old archive and rename the newly written archive, and
     you're done. This is safe but requires a bunch of temporary disk space or heap memory.

     2. Or, you can convert an miniz_zip_reader in-place to an miniz_zip_writer using miniz_zip_writer_init_from_reader(),
     append new files as needed, then finalize the archive which will write an updated central directory to the
     original archive. (This is basically what miniz_zip_add_mem_to_archive_file_in_place() does.) There's a
...
/* Defines to completely disable specific portions of miniz.c: 
   If all macros here are defined the only functionality remaining will be CRC-32, adler-32, _tinfl, and _tdefl. */
...
/* Define MINIZ_NO_MALLOC to disable all calls to malloc, free, and realloc. 
   ...
   functions (such as _tdefl_compress_mem_to_heap() and _tinfl_decompress_mem_to_heap()) won't work. */
...
/* For more compatibility with zlib, miniz.c uses unsigned long for some parameters/struct members. Beware: miniz_ulong can be either 32 or 64-bits! */
typedef unsigned long miniz_ulong;

/* miniz_free() internally uses the MZ_FREE() macro (which by default calls free() unless you've modified the MZ_MALLOC macro) to release a block allocated from the heap. */
void miniz_free(void *p);

#define MZ_ADLER32_INIT (1)
/* miniz_adler32() returns the initial adler-32 value to use when called with ptr==NULL. */
miniz_ulong miniz_adler32(miniz_ulong adler, const unsigned char *ptr, size_t buf_len);

#define MZ_CRC32_INIT (0)
/* miniz_crc32() returns the initial CRC-32 value to use when called with ptr==NULL. */
...
Note that miniz_alloc_func parameter types purpsosely differ from zlib's: items/size is size_t, not unsigned long. */
typedef void *(*miniz_alloc_func)(void *opaque, size_t items, size_t size);
typedef void (*miniz_free_func)(void *opaque, void *address);
typedef void *(*miniz_realloc_func)(void *opaque, void *address, size_t items, size_t size);
...
typedef struct miniz_stream_s
{
    ...
    miniz_alloc_func zalloc; /* optional heap allocation function (defaults to malloc) */
...
/* Quickly resets a compressor without having to reallocate anything. Same as calling miniz_deflateEnd() followed by miniz_deflateInit()/miniz_deflateInit2(). */
int miniz_deflateReset(miniz_streamp pStream);

/* miniz_deflate() compresses the input to output, consuming as much of the input and producing as much output as possible. */
/* Parameters: */
/*   pStream is the stream to read from and write to. You must initialize/update the next_in, avail_in, next_out, and avail_out members. */
/*   flush may be MZ_NO_FLUSH, MZ_PARTIAL_FLUSH/MZ_SYNC_FLUSH, MZ_FULL_FLUSH, or MZ_FINISH. */
/* Return values: */
/*   MZ_OK on success (when flushing, or if more input is needed but not available, and/or there's more output to be written but the output buffer is full). */
/*   MZ_STREAM_END if all input has been consumed and all output bytes have been written. Don't call miniz_deflate() on the stream anymore. */
/*   MZ_STREAM_ERROR if the stream is bogus. */
...
/* miniz_deflateBound() returns a (very) conservative upper bound on the amount of data that could be generated by deflate(), assuming flush is set to only MZ_NO_FLUSH or MZ_FINISH. */
...
/* Returns MZ_OK on success, or one of the error codes from miniz_deflate() on failure. */
...
/* miniz_compressBound() returns a (very) conservative upper bound on the amount of data that could be generated by calling miniz_compress(). */
miniz_ulong miniz_compressBound(miniz_ulong source_len);

/* Initializes a decompressor. */
int miniz_inflateInit(miniz_streamp pStream);

/* miniz_inflateInit2() is like miniz_inflateInit() with an additional option that controls the window size and whether or not the stream has been wrapped with a zlib header/footer: */
/* window_bits must be MZ_DEFAULT_WINDOW_BITS (to parse zlib header/footer) or -MZ_DEFAULT_WINDOW_BITS (raw deflate). */
int miniz_inflateInit2(miniz_streamp pStream, int window_bits);

/* Quickly resets a compressor without having to reallocate anything. Same as calling miniz_inflateEnd() followed by miniz_inflateInit()/miniz_inflateInit2(). */
...
/*   MZ_PARAM_ERROR if one of the parameters is invalid. */
/*   MZ_BUF_ERROR if no forward progress is possible because the input buffer is empty but the inflater needs more input to continue, or if the output buffer is not large enough. Call miniz_inflate() again */
...
/* Returns MZ_OK on success, or one of the error codes from miniz_inflate() on failure. */
...
#define inflateReset miniz_inflateReset
...
typedef int64_t miniz_int64;
typedef uint64_t miniz_uint64;
...
#ifdef MINIZ_NO_TIME
typedef struct miniz_dummy_time_t_tag
{
    int m_dummy;
} miniz_dummy_time_t;
#define MZ_TIME_T miniz_dummy_time_t
#else
...
#define MZ_READ_LE16(p) *((const miniz_uint16 *)(p))
#define MZ_READ_LE32(p) *((const miniz_uint32 *)(p))
#else
#define MZ_READ_LE16(p) ((miniz_uint32)(((const miniz_uint8 *)(p))[0]) | ((miniz_uint32)(((const miniz_uint8 *)(p))[1]) << 8U))
#define MZ_READ_LE32(p) ((miniz_uint32)(((const miniz_uint8 *)(p))[0]) | ((miniz_uint32)(((const miniz_uint8 *)(p))[1]) << 8U) | ((miniz_uint32)(((const miniz_uint8 *)(p))[2]) << 16U) | ((miniz_uint32)(((const miniz_uint8 *)(p))[3]) << 24U))
#endif

#define MZ_READ_LE64(p) (((miniz_uint64)MZ_READ_LE32(p)) | (((miniz_uint64)MZ_READ_LE32((const miniz_uint8 *)(p) + sizeof(miniz_uint32))) << 32U))
...
/* _tdefl_init() compression flags logically OR'd together (low 12 bits contain the max. number of probes per dictionary search): */
...
/* _tdefl_compress_mem_to_heap() compresses a block in memory to a heap block allocated via malloc(). */
/* On entry: */
...
/* _tdefl_compress_mem_to_mem() compresses a block in memory to another block in memory. */
...
/*  The caller must miniz_free() the returned heap block (which will typically be larger than *pLen_out) when it's no longer needed. */
...
typedef miniz_bool (*_tdefl_put_buf_func_ptr)(const void *pBuf, int len, void *pUser);

/* _tdefl_compress_mem_to_output() compresses a block to an output stream. The above helpers use this function internally. */
...
/* The low-level _tdefl functions below may be used directly if the above helper functions aren't flexible enough. The low-level functions don't make any heap allocations, unlike the above helper functions. */
typedef enum {
...
/* _tdefl's compression state structure. */
typedef struct
{
    _tdefl_put_buf_func_ptr m_pPut_buf_func;
    ...
    miniz_uint m_num_flags_left, m_total_lz_bytes, m_lz_code_buf_dict_pos, m_bits_in, m_bit_buffer;
    miniz_uint m_saved_match_dist, m_saved_match_len, m_saved_lit, m_output_flush_ofs, m_output_flush_remaining, m_finished, m_block_index, m_wants_to_finish;
    _tdefl_status m_prev_return_status;
    const void *m_pIn_buf;
    void *m_pOut_buf;
    size_t *m_pIn_buf_size, *m_pOut_buf_size;
    _tdefl_flush m_flush;
...
/* There is no corresponding deinit() function because the _tdefl API's do not dynamically allocate memory. */
/* pBut_buf_func: If NULL, output data will be supplied to the specified callback. In this case, the user should call the _tdefl_compress_buffer() API for compression. */
/* If pBut_buf_func is NULL the user should always call the _tdefl_compress() API. */
...
/* _tdefl_compress_buffer() is only usable when the _tdefl_init() is called with a non-NULL _tdefl_put_buf_func_ptr. */
/* _tdefl_compress_buffer() always consumes the entire input buffer. */
...
/* Create _tdefl_compress() flags given zlib-style compression parameters. */
...
/* Allocate the _tdefl_compressor structure in C so that */
/* non-C language bindings to _tdefl_ API don't need to worry about */
/* structure size and allocation mechanism. */
_tdefl_compressor *_tdefl_compressor_alloc(void);
...
extern "C" {
#endif
/* Decompression flags used by _tinfl_decompress(). */
...
/* _tinfl_decompress_mem_to_heap() decompresses a block in memory to a heap block allocated via malloc(). */
/* On entry: */
/*  pSrc_buf, src_buf_len: Pointer and size of the Deflate or zlib source data to decompress. */
/* On return: */
/*  Function returns a pointer to the decompressed data, or NULL on failure. */
/*  *pOut_len will be set to the decompressed data's size, which could be larger than src_buf_len on uncompressible data. */
/*  The caller must call miniz_free() on the returned block when it's no longer needed. */
void *_tinfl_decompress_mem_to_heap(const void *pSrc_buf, size_t src_buf_len, size_t *pOut_len, int flags);

/* _tinfl_decompress_mem_to_mem() decompresses a block in memory to another block in memory. */
...
/* _tinfl_decompress_mem_to_callback() decompresses a block in memory to an internal 32KB buffer, and a user provided callback function will be called to flush the buffer. */
/* Returns 1 on success or 0 on failure. */
typedef int (*_tinfl_put_buf_func_ptr)(const void *pBuf, int len, void *pUser);
...
struct _tinfl_decompressor_tag;
typedef struct _tinfl_decompressor_tag _tinfl_decompressor;

#ifndef MINIZ_NO_MALLOC
/* Allocate the _tinfl_decompressor structure in C so that */
/* non-C language bindings to _tinfl_ API don't need to worry about */
/* structure size and allocation mechanism. */
_tinfl_decompressor *_tinfl_decompressor_alloc(void);
...
typedef enum {
    ...
    /* This flags indicate the inflator has somehow failed (bad code, corrupted input, etc.). If you call it again without resetting via _tinfl_init() it it'll just keep on returning the same status failure code. */
...
#define _tinfl_init(r)     \
    do                    \
    {                     \
        (r)->m_state = 0; \
    }                     \
    MZ_MACRO_END
#define _tinfl_get_adler32(r) (r)->m_check_adler32
...
typedef struct
{
    ...
    miniz_int16 m_look_up[TINFL_FAST_LOOKUP_SIZE], m_tree[TINFL_MAX_HUFF_SYMBOLS_0 * 2];
} _tinfl_huff_table;
...
typedef miniz_uint64 _tinfl_bit_buf_t;
#define TINFL_BITBUF_SIZE (64)
#else
typedef miniz_uint32 _tinfl_bit_buf_t;
#define TINFL_BITBUF_SIZE (32)
#endif

struct _tinfl_decompressor_tag
{
    miniz_uint32 m_state, m_num_bits, m_zhdr0, m_zhdr1, m_z_adler32, m_final, m_type, m_check_adler32, m_dist, m_counter, m_num_extra, m_table_sizes[TINFL_MAX_HUFF_TABLES];
    _tinfl_bit_buf_t m_bit_buf;
    size_t m_dist_from_out_buf_start;
    _tinfl_huff_table m_tables[TINFL_MAX_HUFF_TABLES];
    miniz_uint8 m_raw_header[4], m_len_codes[TINFL_MAX_HUFF_SYMBOLS_0 + TINFL_MAX_HUFF_SYMBOLS_1 + 137];
};
...
typedef struct
{
    ...
    miniz_uint64 m_central_dir_ofs;
    ...
    miniz_bool m_is_directory;

    /* MZ_TRUE if the entry uses encryption/strong encryption (which miniz_zip doesn't support) */
    miniz_bool m_is_encrypted;

    /* MZ_TRUE if the file is not encrypted, a patch file, and if it uses a compression method we support. */
    miniz_bool m_is_supported;
...
typedef size_t (*miniz_file_read_func)(void *pOpaque, miniz_uint64 file_ofs, void *pBuf, size_t n);
typedef size_t (*miniz_file_write_func)(void *pOpaque, miniz_uint64 file_ofs, const void *pBuf, size_t n);
...
typedef enum {
    ...
    MZ_ZIP_FLAG_VALIDATE_LOCATE_FILE_FLAG = 0x1000, /* if enabled, miniz_zip_reader_locate_file() will be called on each file as its validated to ensure the func finds the file in the central dir (intended for testing) */
    MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY = 0x2000,     /* validate the local headers, but don't decompress the entire file and check the crc32 */
    MZ_ZIP_FLAG_WRITE_ZIP64 = 0x4000,               /* always use the zip64 file format, instead of the original zip file format with automatic switch to zip64. Use as flags parameter with miniz_zip_writer_init*_v2 */
...
/* miniz error codes. Be sure to update miniz_zip_get_error_string() if you add or modify this enum. */
typedef enum {
    MZ_ZIP_NO_ERROR = 0,
...
typedef struct
{
    miniz_zip_archive *pZip;
    miniz_uint flags;

    int status;
#ifndef MINIZ_DISABLE_ZIP_READER_CRC32_CHECKS
    miniz_uint file_crc32;
#endif
    miniz_uint64 read_buf_size, read_buf_ofs, read_buf_avail, comp_remaining, out_buf_ofs, cur_file_ofs;
...
} miniz_zip_reader_extract_iter_state;
...
/* The FILE will NOT be closed when miniz_zip_reader_end() is called. */
miniz_bool miniz_zip_reader_init_cfile(miniz_zip_archive *pZip, MZ_FILE *pFile, miniz_uint64 archive_size, miniz_uint flags);
#endif

/* Ends archive reading, freeing all allocations, and closing the input archive file if miniz_zip_reader_init_file() was used. */
miniz_bool miniz_zip_reader_end(miniz_zip_archive *pZip);

/* -------- ZIP reading or writing */

/* Clears a miniz_zip_archive struct to all zeros. */
/* Important: This must be done before passing the struct to any miniz_zip functions. */
...
/* All miniz_zip funcs set the m_last_error field in the miniz_zip_archive struct. These functions retrieve/manipulate this field. */
...
int miniz_zip_reader_locate_file(miniz_zip_archive *pZip, const char *pName, const char *pComment, miniz_uint flags);
int miniz_zip_reader_locate_file_v2(miniz_zip_archive *pZip, const char *pName, const char *pComment, miniz_uint flags, miniz_uint32 *file_index);
...
/* The memory will be allocated via the miniz_zip_archive's alloc/realloc functions. */
...
miniz_bool miniz_zip_reader_extract_to_cfile(miniz_zip_archive *pZip, miniz_uint file_index, MZ_FILE *File, miniz_uint flags);
...
#if 0
/* TODO */
	typedef void *miniz_zip_streaming_extract_state_ptr;
	miniz_zip_streaming_extract_state_ptr miniz_zip_streaming_extract_begin(miniz_zip_archive *pZip, miniz_uint file_index, miniz_uint flags);
	uint64_t miniz_zip_streaming_extract_get_size(miniz_zip_archive *pZip, miniz_zip_streaming_extract_state_ptr pState);
	uint64_t miniz_zip_streaming_extract_get_cur_ofs(miniz_zip_archive *pZip, miniz_zip_streaming_extract_state_ptr pState);
	miniz_bool miniz_zip_streaming_extract_seek(miniz_zip_archive *pZip, miniz_zip_streaming_extract_state_ptr pState, uint64_t new_ofs);
	size_t miniz_zip_streaming_extract_read(miniz_zip_archive *pZip, miniz_zip_streaming_extract_state_ptr pState, void *pBuf, size_t buf_size);
	miniz_bool miniz_zip_streaming_extract_end(miniz_zip_archive *pZip, miniz_zip_streaming_extract_state_ptr pState);
#endif
...
/* Validates an entire archive by calling miniz_zip_validate_file() on each file. */
...
/* Universal end function - calls either miniz_zip_reader_end() or miniz_zip_writer_end(). */
...
/*Set pZip->m_pWrite (and pZip->m_pIO_opaque) before calling miniz_zip_writer_init or miniz_zip_writer_init_v2*/
/*The output is streamable, i.e. file_ofs in miniz_file_write_func always increases only by n*/
...
/* For archives opened using miniz_zip_reader_init_file, pFilename must be the archive's filename so it can be reopened for writing. If the file can't be reopened, miniz_zip_reader_end() will be called. */
/* For archives opened using miniz_zip_reader_init_mem, the memory block must be growable using the realloc callback (which defaults to realloc unless you've overridden it). */
/* Finally, for archives opened using miniz_zip_reader_init, the miniz_zip_archive's user provided m_pWrite function cannot be NULL. */
...
/* level_and_flags - compression level (0-10, see MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc.) logically OR'd with zero or more miniz_zip_flags, or just set to MZ_DEFAULT_COMPRESSION. */
...
/* Like miniz_zip_writer_add_mem(), except you can specify a file comment field, and optionally supply the function with already compressed data. */
...
miniz_bool miniz_zip_writer_add_mem_ex_v2(miniz_zip_archive *pZip, const char *pArchive_name, const void *pBuf, size_t buf_size, const void *pComment, miniz_uint16 comment_size, miniz_uint level_and_flags,
                                    miniz_uint64 uncomp_size, miniz_uint32 uncomp_crc32, MZ_TIME_T *last_modified, const char *user_extra_data_local, miniz_uint user_extra_data_local_len,
                                    const char *user_extra_data_central, miniz_uint user_extra_data_central_len);

/* Adds the contents of a file to an archive. This function also records the disk file's modified time into the archive. */
/* File data is supplied via a read callback function. User miniz_zip_writer_add_(c)file to add a file directly.*/
miniz_bool miniz_zip_writer_add_read_buf_callback(miniz_zip_archive *pZip, const char *pArchive_name, miniz_file_read_func read_callback, void* callback_opaque, miniz_uint64 size_to_add,
	const MZ_TIME_T *pFile_time, const void *pComment, miniz_uint16 comment_size, miniz_uint level_and_flags, const char *user_extra_data_local, miniz_uint user_extra_data_local_len,
	const char *user_extra_data_central, miniz_uint user_extra_data_central_len);
...
/* Like miniz_zip_writer_add_file(), except the file data is read from the specified FILE stream. */
miniz_bool miniz_zip_writer_add_cfile(miniz_zip_archive *pZip, const char *pArchive_name, MZ_FILE *pSrc_file, miniz_uint64 size_to_add,
                                const MZ_TIME_T *pFile_time, const void *pComment, miniz_uint16 comment_size, miniz_uint level_and_flags, const char *user_extra_data_local, miniz_uint user_extra_data_local_len,
                                const char *user_extra_data_central, miniz_uint user_extra_data_central_len);
#endif
...
/* After an archive is finalized, the only valid call on the miniz_zip_archive struct is miniz_zip_writer_end(). */
/* An archive must be manually finalized by calling this function for it to be valid. */
miniz_bool miniz_zip_writer_finalize_archive(miniz_zip_archive *pZip);

/* Finalizes a heap archive, returning a poiner to the heap block and its size. */
/* The heap block will be allocated using the miniz_zip_archive's alloc/realloc callbacks. */
miniz_bool miniz_zip_writer_finalize_heap_archive(miniz_zip_archive *pZip, void **ppBuf, size_t *pSize);

/* Ends archive writing, freeing all allocations, and closing the output file if miniz_zip_writer_init_file() was used. */
/* Note for the archive to be valid, it *must* have been finalized before ending (this function will not do it for you). */
miniz_bool miniz_zip_writer_end(miniz_zip_archive *pZip);

/* -------- Misc. high-level helper functions: */

/* miniz_zip_add_mem_to_archive_file_in_place() efficiently (but not atomically) appends a memory blob to a ZIP archive. */
